...
describe("Fix bug https://gitlab.com/Schroedinger1/backend/-/issues/70", () => {
    test("Fix bug 70 - empty free style questions", async (done) => {
        const username = uuid();
        const password = uuid();
        const email = uuid();
        ...
        expect(getSubmissions.body[0].freestyle_answers.length).toBe(0);
        expect(getSubmissions.body[1].freestyle_answers.length).toBe(0);
    ...
    test("Fix bug 70 - empty constrained style questions", async (done) => {
        const username = uuid();
        const password = uuid();
        const email = uuid();
        const registerUser = await testUtils.registerUser(username, `${email}@mail.com`, password);
        ...
        expect(getSubmissions.body[0].constrained_answers.length).toBe(0);
        expect(getSubmissions.body[1].constrained_answers.length).toBe(0);

        done();
    });


    test("Fix bug 70 - no questions at all", async (done) => {
        const username = uuid();
        const password = uuid();
        const email = uuid();
        const registerUser = await testUtils.registerUser(username, `${email}@mail.com`, password);
        ...
        const createdSurvey = await request
            ...
            .send(securedPayload)
            .set("authorization", jwtToken);
        expect(createdSurvey.status).toEqual(400);
